...
app.get("/api/v1/hotels/:id", async (req, res) => {
  ...
  try {
    const hotel = await db.query(
      "select * from hotels left join (select hotel_id, COUNT(*), TRUNC(AVG(rating),1) as average_rating from reviews group by hotel_id) reviews on restaurants.id = reviews.restaurant_id where id = $1",
      [req.params.id]
    );
