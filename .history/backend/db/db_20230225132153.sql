...
CREATE TABLE reviews (
  id BIGSERIAL NOT NULL PRIMARY KEY,
    hotels_id BIGINT NOT NULL REFERENCES hotels(id) ON DELETE CASCADE,
...
-- ALTER TABLE reviews  ADD  constraint  hotels_id foreign key ( hotels_id) references hotels( id ) on delete cascade;

 
-- ALTER TABLE reviews foreign key ( hotels_id) REFERENCES hotels(id) ON DELETE CASCADE ON UPDATE SET NULL (hotels_id);

select * 
from hotels
    left join(
        select hotels_id,
            count(*), TRUNC(AVG(rating), 1)) as average_rating from reviews group by hotels_id) reviews on hotels.id = reviews.hotels_id order by average_rating asc;
