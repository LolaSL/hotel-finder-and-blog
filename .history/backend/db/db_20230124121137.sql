...
from hotels
    ...
    select * from hotels left join (select hotels_id, count(*), trunc(AVG(rating), 1) as rating from reviews GROUP by hotels_id reviews on hotels.id = reviews.hotels_id;
