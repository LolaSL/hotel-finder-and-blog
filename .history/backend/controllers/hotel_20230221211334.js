...
exports.getHotelById = async (req, res) => {
  ...
  try {
    const hotel = await pool.query(
      "select * from hotels left join (select hotels_id, COUNT(*), TRUNC(AVG(rating),1) as average_rating from reviews) reviews on hotels.id = reviews.hotels_id where id = $1",
      [req.params.id]
    );
