...
exports.getHotels = async (req, res) => {
  try {
    const hotelRatingsData = await pool.query(
      "select * from hotels left join (select hotels_id, COUNT(*), TRUNC(AVG(rating),1) as average_rating from reviews group by hotels_id) reviews on hotels.id = reviews.hotels_id order by average_rating desc;"
    );
    res.status(200).json({
      status: "success",
