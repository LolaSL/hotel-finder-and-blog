...
const jwt_decode = require('jwt-decode');
 ...
 exports.getUsers = async (req, res) => {
  try {
    const { rows } = await db.query('select user_id, name, email, created_at from users, is_admin')
...
exports.register = async (req, res) => {
  ...
  try {
    ...
    await db.query('insert into users(name, email, password, is_admin) values ($1 , $2, $3, $4)', [
      name,
      email,
      hashedPassword,
    ])
...
exports.login = async (req, res) => {
  ...
  try {
    const access_token = await sign(payload, SECRET)
