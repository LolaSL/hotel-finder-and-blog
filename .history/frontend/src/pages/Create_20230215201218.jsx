...
//   const state = useLocation().state;
  const { place } = props;
  const [value, setValue] = useState(place?.title || "");
  const [title, setTitle] = useState(place?.description || "");
  const [file, setFile] = useState(null);
  const [where_to_stay, setWhereToStay] = useState(place?.where_to_stay || "");
  ...
  const handleClick = async (e) => {
    ...
    try {
      state
        ? await axios.put(`http://localhost:8090/api/v1/places${splace.id}`, {
            title,
            where_to_stay,
            description: value,
